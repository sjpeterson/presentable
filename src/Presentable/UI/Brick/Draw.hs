...
import Presentable.Data.TextBlock (
...
drawUI appEnv appState =
  ...
  where
    inner =
        vBox
            [ padBottom Max $ padRight Max $ slide
            , copyrightNotice
            ]
    slide =
        either drawError (drawSlide columns) $
...
drawSlide _ (TitleSlide title subtitle) = case subtitle of
    ...
    Just s ->
        C.center $
            vBox
                [ padBottom (Pad 1) titleWidget
                , C.hCenter $ withAttr subtitleAttr $ txt s
                ]
  where
    titleWidget = C.hCenter $ withAttr titleAttr $ txt title
drawSlide columns (SingleContentSlide title content) =
    padBottom Max $
        vBox
            [ padBottom (Pad 1) $ withAttr slideTitleAttr $ txt title
...
drawContent columns (BulletListContent (BulletList items)) =
    padRight Max $
        vBox $
            map (drawBulletListItem columns topLevelMarker) (NE.toList items)
drawContent columns (PlainTextContent paragraphs) =
    padRight Max $
        vBox $
            intersperse (txt " ") $
