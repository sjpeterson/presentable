...
import Presentable.Data.Wrappable ( Wrappable ( wrapAt, wrapRelaxedAt )
                                  , WrappingError
                                  )
...
-- | Wrap particles of a text block to the given number of columns.
wrapParticlesRelaxedAt :: Int
                       -> NonEmpty TaggedText
                       -> NonEmpty (NonEmpty TaggedText)
wrapParticlesRelaxedAt c (p :| ps) = NE.reverse . fmap (NE.reverse . fst) $
