...
spec = do
    describe "wrapAt" $ do
        it "leaves a short enough plain text line intact" $ do
            wrapAt 10 (plainTextBlock "Short text")
                `shouldBe` Right [plainTextBlock "Short text"]
        it "wraps plain text on exact word boundary" $ do
            wrapAt 5 (plainTextBlock "Short text")
                `shouldBe` Right [plainTextBlock "Short", plainTextBlock "text"]
        it "wraps plain text on preceeding word boundary" $ do
            wrapAt 7 (plainTextBlock "Short text")
                `shouldBe` Right [plainTextBlock "Short", plainTextBlock "text"]
        it "wraps from the beginning" $ do
            wrapAt 18 (plainTextBlock "Slightly longer text")
                `shouldBe` Right [plainTextBlock "Slightly longer", plainTextBlock "text"]
    ...
    describe "wrapRelaxedAt" $ do
        it "leaves a short enough plain text line intact" $ do
            wrapRelaxedAt 10 (plainTextBlock "Short text")
                `shouldBe` [plainTextBlock "Short text"]
        it "wraps plain text on exact word boundary" $ do
            wrapRelaxedAt 5 (plainTextBlock "Short text")
                `shouldBe` [plainTextBlock "Short", plainTextBlock "text"]
        it "wraps plain text on preceeding word boundary" $ do
            wrapRelaxedAt 7 (plainTextBlock "Short text")
                `shouldBe` [plainTextBlock "Short", plainTextBlock "text"]
        it "wraps from the beginning" $ do
            wrapRelaxedAt 18 (plainTextBlock "Slightly longer text")
                `shouldBe` [plainTextBlock "Slightly longer", plainTextBlock "text"]
